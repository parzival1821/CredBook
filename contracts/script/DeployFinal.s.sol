...
contract DeployFinal is Script {
    ...
    uint256 constant INITIAL_LIQUIDITY = 10 * 1e6; // 10 USDC
    ...
    function _fundPools(uint256 key, Deployments memory d, address deployer) internal {
        ...
        uint256 totalNeeded = INITIAL_LIQUIDITY * 4;
        
        console.log("Deployer USDC balance:", balance / 1e6);
        console.log("Total needed:", totalNeeded / 1e6, "USDC");
        require(balance >= totalNeeded, "Insufficient USDC balance");
        
        // Fund Pool 0
        vm.broadcast(key);
        usdc.approve(address(d.pool0), INITIAL_LIQUIDITY);
        vm.broadcast(key);
        d.pool0.supply(1, INITIAL_LIQUIDITY, 0, deployer, "");
        console.log("Pool 0 funded with 10k USDC");
        
        // Fund Pool 1
        vm.broadcast(key);
        usdc.approve(address(d.pool1), INITIAL_LIQUIDITY);
        vm.broadcast(key);
        d.pool1.supply(2, INITIAL_LIQUIDITY, 0, deployer, "");
        console.log("Pool 1 funded with 10k USDC");
        
        // Fund Pool 2
        vm.broadcast(key);
        usdc.approve(address(d.pool2), INITIAL_LIQUIDITY);
        vm.broadcast(key);
        d.pool2.supply(3, INITIAL_LIQUIDITY, 0, deployer, "");
        console.log("Pool 2 funded with 10k USDC");
        
        // Fund Pool 3
        vm.broadcast(key);
        usdc.approve(address(d.pool3), INITIAL_LIQUIDITY);
        vm.broadcast(key);
        d.pool3.supply(4, INITIAL_LIQUIDITY, 0, deployer, "");
        console.log("Pool 3 funded with 10k USDC");
    ...
    function _logAddresses(Deployments memory d) internal view {
        ...
        console.log("Pool 0:", address(d.pool0), "(10k USDC funded)");
        console.log("Pool 1:", address(d.pool1), "(10k USDC funded)");
        console.log("Pool 2:", address(d.pool2), "(10k USDC funded)");
        console.log("Pool 3:", address(d.pool3), "(10k USDC funded)");
        console.log("\nTotal Liquidity: 40k USDC");
    }
}